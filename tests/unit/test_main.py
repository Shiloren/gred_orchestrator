...
def test_panic_catcher_middleware(auth_client):
    with patch('tools.repo_orchestrator.security.load_security_db', return_value={"panic_mode": False}):
...
def test_panic_mode_check_middleware(auth_client):
    with patch('tools.repo_orchestrator.security.load_security_db', return_value={"panic_mode": True}):
...
def test_allow_options_preflight(auth_client):
    ...
    response = auth_client.options("/ui/status", headers={
