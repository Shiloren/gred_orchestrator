...
def test_get_ui_status(client):
    with patch('tools.repo_orchestrator.routes.FileService.tail_audit_lines', return_value=["audit line"]):
...
def test_get_ui_audit(client):
    with patch('tools.repo_orchestrator.routes.FileService.tail_audit_lines', return_value=["l1", "l2"]):
...
def test_list_repos(client):
    with patch('tools.repo_orchestrator.routes.RepoService.list_repos', return_value=[RepoEntry(name="r1", path="C:\\Users\\shilo\\repo"), RepoEntry(name="empty", path="")]):
        with patch('tools.repo_orchestrator.routes.RepoService.ensure_repo_registry', return_value={"active_repo": "C:\\Users\\shilo\\repo"}):
...
def test_get_active_repo(client):
    with patch('tools.repo_orchestrator.routes.load_repo_registry', return_value={"active_repo": "/mock/active"}):
...
def test_get_security_events(client):
    with patch('tools.repo_orchestrator.routes.load_security_db', return_value={"panic_mode": False, "recent_events": []}):
        response = client.get("/ui/security/events")
        assert response.status_code == 200
        assert response.json()["panic_mode"] is False

def test_security_resolve_success(client):
    with patch('tools.repo_orchestrator.routes.load_security_db', return_value={"panic_mode": True, "recent_events": [{"resolved": False}]}):
...
def test_vitaminize_repo_success(client, tmp_path):
    ...
    repo.mkdir()
    with patch('tools.repo_orchestrator.routes.REPO_ROOT_DIR', tmp_path):
        with patch('tools.repo_orchestrator.routes.RepoService.vitaminize_repo', return_value=["vit"]):
    ...
    with patch('tools.repo_orchestrator.routes.get_active_repo_dir', return_value=tmp_path):
        with patch('tools.repo_orchestrator.routes.validate_path', return_value=tmp_path):
            with patch('tools.repo_orchestrator.routes.ALLOWLIST_REQUIRE', True):
                with patch('tools.repo_orchestrator.routes.get_allowed_paths', return_value={tmp_path / "allowed.txt"}):
                    response = client.get("/tree?path=.")
                    assert response.status_code == 200
...
def test_get_file_success(client, tmp_path):
    ...
    with patch('tools.repo_orchestrator.routes.get_active_repo_dir', return_value=tmp_path):
        with patch('tools.repo_orchestrator.routes.validate_path', return_value=f):
            with patch('tools.repo_orchestrator.routes.FileService.get_file_content', return_value=("content", "hash")):
...
def test_get_file_exception(client, tmp_path):
    ...
    with patch('tools.repo_orchestrator.routes.get_active_repo_dir', return_value=tmp_path):
        with patch('tools.repo_orchestrator.routes.validate_path', return_value=f):
            with patch('tools.repo_orchestrator.routes.FileService.get_file_content', side_effect=Exception("oops")):
                response = client.get("/file?path=test.py")
                assert response.status_code == 500

def test_search(client, tmp_path):
    with patch('tools.repo_orchestrator.routes.get_active_repo_dir', return_value=tmp_path):
        with patch('tools.repo_orchestrator.routes.RepoService.perform_search', return_value=[{"file": "a.py"}]):
...
def test_diff_success(client, tmp_path):
    with patch('tools.repo_orchestrator.routes.get_active_repo_dir', return_value=tmp_path):
        with patch('tools.repo_orchestrator.services.git_service.GitService.get_diff', return_value="diff data"):
...
def test_diff_truncated(client, tmp_path):
    with patch('tools.repo_orchestrator.routes.get_active_repo_dir', return_value=tmp_path):
        ...
        long_diff = "line\n" * 100
        with patch('tools.repo_orchestrator.services.git_service.GitService.get_diff', return_value=long_diff):
...
def test_diff_error(client, tmp_path):
    with patch('tools.repo_orchestrator.routes.get_active_repo_dir', return_value=tmp_path):
        with patch('tools.repo_orchestrator.services.git_service.GitService.get_diff', side_effect=Exception("git fail")):
            response = client.get("/diff")
            assert response.status_code == 400
