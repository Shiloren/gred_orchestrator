...
def test_tail_audit_lines_no_file():
    with patch("tools.gimo_server.services.file_service.AUDIT_LOG_PATH", Path("/nonexistent/audit.log")):
...
def test_get_file_content(tmp_path):
    ...
    with patch("tools.gimo_server.services.file_service.SnapshotService.create_snapshot") as mock_snap, \
         patch("tools.gimo_server.services.file_service.audit_log") as mock_audit, \
         patch("tools.gimo_server.services.file_service.redact_sensitive_data", side_effect=lambda x: x), \
         patch("tools.gimo_server.services.file_service.MAX_LINES", 500), \
         patch("tools.gimo_server.services.file_service.MAX_BYTES", 250000):
...
def test_get_file_content_truncation(tmp_path):
    ...
    with patch("tools.gimo_server.services.file_service.SnapshotService.create_snapshot", return_value=source_file), \
         patch("tools.gimo_server.services.file_service.audit_log"), \
         patch("tools.gimo_server.services.file_service.redact_sensitive_data", side_effect=lambda x: x), \
         patch("tools.gimo_server.services.file_service.MAX_LINES", 10), \
         patch("tools.gimo_server.services.file_service.MAX_BYTES", 250000):
