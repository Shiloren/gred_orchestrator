...
@pytest.fixture
def mock_gics():
    return MagicMock(spec=GicsService)
...
def cost_storage(mock_gics):
    ...
    storage = CostStorage(conn, gics=mock_gics)
...
def test_save_cost_event(cost_storage, mock_gics):
    ...
    cursor = cost_storage._conn.execute("SELECT * FROM cost_events WHERE id = ?", ("evt_1",))
    row = cursor.fetchone()
    assert row is not None
    assert row[0] == "evt_1"
    assert row[9] == 0.015

    mock_gics.put.assert_called_once()
    call_args = mock_gics.put.call_args
    assert call_args[0][0].startswith("ce:w1:n1:")
    assert call_args[0][1]["id"] == "evt_1"
...
def test_roi_leaderboard(cost_storage):
    now = datetime.now(timezone.utc)
    # haiku: cheap + decent quality => high ROI
    ...
    # opus: expensive + high quality => lower ROI
    ...
    # haiku should rank first (higher ROI)
