...
export const InspectPanel: React.FC<InspectPanelProps> = ({
    ...
    // Reset view when node matches sidebar tab changes
    React.useEffect(() => {
        if (sidebarTab === 'logs') setView('logs');
        else if (sidebarTab === 'graph') {
            if (selectedNodeId) {
                // Keep current view if it's node-specific, otherwise default to plan
                if (!['plan', 'quality', 'overview', 'chat'].includes(view)) {
                    setView('plan');
                }
            } else {
                setView('overview');
    ...
    }, [sidebarTab, selectedNodeId]);

    const getPanelTitle = () => {
        if (selectedNodeId && sidebarTab === 'graph') return `Node: ${selectedNode?.data?.label || selectedNodeId}`;
        switch (sidebarTab) {
            case 'graph': return 'Graph Overview';
            case 'plans': return 'Orchestrator Plans';
    ...
    return (
        <aside className="w-[380px] bg-[#0a0a0a] border-l border-[#2c2c2e] flex flex-col shrink-0 overflow-hidden shadow-2xl z-40">
            ...
            {/* Tab Navigation for Selected Node */}
            {selectedNodeId && sidebarTab === 'graph' && (
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-6">
                {sidebarTab === 'maintenance' && children}

                {sidebarTab === 'logs' && <LiveLogs />}

                {sidebarTab === 'graph' && (
                    <>
                        {selectedNodeId ? (
                            <>
                                {/* Pending Questions Escalation */}
