...
logger = logging.getLogger("orchestrator.adapters.local_llm")

class LocalLLMSession(AgentSession):
    """Session for a Local LLM (OpenAI-compatible) agent."""
...
class LocalLLMAdapter(AgentAdapter):
    """Adapter for Local LLMs via OpenAI-compatible API."""
    ...
    async def spawn(
        ...
        logger.info(f"Spawning Local LLM Session: {self.model_name}")
